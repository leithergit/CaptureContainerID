...
public:
    BOOL TransmiteData(const std::wstring& wstrUrl, EType eType, DWORD dwTimeout,TCHAR *szAccount = NULL,TCHAR *szPassword = NULL);

    VOID ClearEvn();

    BOOL ReceiveData(LPBYTE lpBuffer, DWORD& dwBufferSize);
private:
    BOOL InitializeHttp( const std::wstring& wstrUrl, DWORD dwTimeout, EType eType,TCHAR *szAccount = NULL,TCHAR *szPassword = NULL);
