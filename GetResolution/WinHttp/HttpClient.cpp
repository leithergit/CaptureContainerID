...
BOOL CHttpClient::InitializeHttp( const std::wstring& wstrUrl, DWORD dwTimeout, EType eType,TCHAR *szAccount ,TCHAR *szPassword )
{
    BOOL bSuc = FALSE;
    do {
		...
		if (szAccount && _tcslen(szAccount))
			m_strAccount = szAccount;
		if (szPassword && _tcslen(szPassword))
...
BOOL CHttpClient::TransmiteData( const std::wstring& wstrUrl, EType eType, DWORD dwTimeout,TCHAR *szAccount ,TCHAR *szPassword )
{
    BOOL bSuc = FALSE;
    do {
